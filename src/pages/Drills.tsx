...
/**
 * Partition Drills — T3's incident-reading exam. Three scripted cards,
...
export default function Drills() {
  ...
  return (
    <div className="mx-auto max-w-app px-6 pb-24 pt-16 lg:px-12">
      <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-accent">T3 · partition drills</p>
      <h1 className="mt-3 text-4xl font-semibold tracking-tight text-text-1">Read the dying cluster.</h1>
      <p className="mt-4 max-w-2xl text-body-lg text-text-2">
        Three incidents, real-shape telemetry, no simulator to hide behind. For each card: read the
        briefing and the curves, call the root cause and the mitigation. Diagnose all three — the
              ...
              <>
                ...
                {solved.length >= DRILLS.length && (
                  <p className="mt-3 font-mono text-[12px] text-accent">
                    all three diagnosed — the cluster fears you. close the loop on{' '}
                    <Link to="/cluster" className="underline">The Cluster</Link> or finish the{' '}
                    <Link to="/labs/linearizable-kv" className="underline">capstone lab</Link>.
                  </p>
                )}
              </>
            ) : (
